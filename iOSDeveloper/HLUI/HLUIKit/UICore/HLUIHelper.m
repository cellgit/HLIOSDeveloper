...
@implementation HLUIHelper (DynamicType)
...
+ (CGFloat)heightForDynamicTypeCell:(NSArray *)heights {
    NSNumber *index = [HLUIHelper preferredContentSizeLevel];
...
@implementation HLUIHelper (Keyboard)
...
- (void)handleKeyboardWillShow:(NSNotification *)notification {
    ...
    self.lastKeyboardHeight = [HLUIHelper keyboardHeightWithNotification:notification];
}
...
+ (BOOL)isKeyboardVisible {
    BOOL visible = [[HLUIHelper sharedInstance] isKeyboardVisible];
    return visible;
}
...
+ (CGFloat)lastKeyboardHeightInApplicationWindowWhenVisible {
    return [[HLUIHelper sharedInstance] lastKeyboardHeight];
}
...
+ (CGFloat)keyboardHeightWithNotification:(NSNotification *)notification {
    return [HLUIHelper keyboardHeightWithNotification:notification inView:nil];
}
...
+ (UIViewAnimationOptions)keyboardAnimationOptionsWithNotification:(NSNotification *)notification {
    UIViewAnimationOptions options = [HLUIHelper keyboardAnimationCurveWithNotification:notification]<<16;
    return options;
}
...
@implementation HLUIHelper (AudioSession)
...
@implementation HLUIHelper (UIGraphic)
...
//@implementation HLUIHelper (Device)
//
...
//+ (BOOL)isIPadPro {
//    if (isIPadPro < 0) {
//        isIPadPro = [HLUIHelper isIPad] ? (DEVICE_WIDTH == 1024 && DEVICE_HEIGHT == 1366 ? 1 : 0) : 0;
//    }
//    return isIPadPro > 0;
//}
//
...
@implementation HLUIHelper (Orientation)
...
+ (CGAffineTransform)transformForCurrentInterfaceOrientation {
    return [HLUIHelper transformWithInterfaceOrientation:[[UIApplication sharedApplication] statusBarOrientation]];
}

+ (CGAffineTransform)transformWithInterfaceOrientation:(UIInterfaceOrientation)orientation {
    CGFloat angle = [HLUIHelper angleForTransformWithInterfaceOrientation:orientation];
...
@implementation HLUIHelper (UIApplication)
...
#warning HLUISpringAnimationKey = @"HLUISpringAnimationKey"; 由 QMUISpringAnimationKey = @"QMUISpringAnimationKey";改写
NSString *const HLUISpringAnimationKey = @"HLUISpringAnimationKey";

@implementation HLUIHelper (Animation)
...
+ (void)actionSpringAnimationForView:(UIView *)view {
    ...
    [view.layer addAnimation:springAnimation forKey:HLUISpringAnimationKey];
}

@end


@implementation HLUIHelper (Log)
...
- (void)printLogWithCalledFunction:(nonnull const char *)func log:(nonnull NSString *)log, ... {
    ...
    if ([self.helperDelegate respondsToSelector:@selector(HLUIHelperPrintLog:)]) {
        [self.helperDelegate HLUIHelperPrintLog:[NSString stringWithFormat:@"QMUI - %@. Called By %s", logString, func]];
    } else {
...
@implementation HLUIHelper

+ (instancetype)sharedInstance {
    static dispatch_once_t onceToken;
    static HLUIHelper *instance = nil;
    dispatch_once(&onceToken,^{
